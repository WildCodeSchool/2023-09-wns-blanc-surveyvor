...
test.describe("Add a new survey", () => {
  ...
  test("03. Go to the new survey page", async () => {
    ...
    await expect(page).toHaveURL(new RegExp("^http://localhost:3000/surveys/"));
  });
  ...
  test("04. Create a new survey", async () => {
    ...
    await descriptionInput.fill("Test de nouvelle description de formulaire");
  });
  ...
  test("05. Add a question", async () => {
    ...
    const questionDescriptionInput = await page.locator(
      'form textarea[name="question-description"]'
    );
    await questionDescriptionInput.click();
    await questionDescriptionInput.fill(
      "Test de nouvelle description de question"
    );
